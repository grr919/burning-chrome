...
function App() {
  ...
  const enterBuildingAndStreetView = (cell: GridCellBuilding) => {
  ...
  const handleGridCellClick = (cell: GridCellBuilding) => {
    enterBuildingAndStreetView(cell);
  };
  ...
  useEffect(() => {
    ...
    if (!container || layoutMode !== 'street') {
      return;
    }
        ...
        {buildingView ? (
          <div className="flex-1 min-h-0 flex flex-col gap-3 lg:flex-row">
            <div className="relative flex-1 min-h-[260px] lg:flex-[1.35] rounded-xl overflow-hidden border border-gray-700 bg-[#eaf6ff]">
              <Canvas
                key={`building-street-${viewResetKey}-${buildingView.ipAddress}`}
            ...
            <div className="min-h-0 lg:w-[380px] bg-white text-black border border-gray-300 rounded-xl shadow-lg p-3 overflow-auto">
              <div className="font-bold text-lg">Building and Street View: {buildingView.ipAddress}</div>
              <div className="text-sm text-gray-600 mt-1">
                Use Return to grid to leave Building and Street View.
              </div>
                  ...
                  {buildingDirectoryEntries.length > 0 ? (
                    <div className="mt-1 space-y-1">
                      {buildingDirectoryEntries.map((entry) => (
                        <a
                          key={`${entry.hostname}-${entry.url}`}
                          href={entry.url}
                          target="_blank"
                          rel="noreferrer"
                          className="block break-all text-blue-700 underline decoration-blue-400 underline-offset-2"
                        >
                          {entry.hostname}
                        </a>
                      ))}
                    ...
                    <div className="mt-1 text-gray-600">No websites identified.</div>
                  )}
                </div>
